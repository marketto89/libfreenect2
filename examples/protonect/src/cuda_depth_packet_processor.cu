                   ...
                   pcl::PointXYZRGB* cloud_out)
{
   ...
   pcl::PointXYZRGB& p = cloud_out[i];
    ...
    p.x = ((x - cx) * depth / fx) / 1000.0f;
    p.y = ((y - cy) * depth / fy) / 1000.0f;
    p.z = depth / 1000.0f;
    p.r = p.b = p.g = static_cast<uint8_t>(ir_data[i]);
}
...
#define COPY(upper, lower) cudaMemcpyToSymbolAsync(upper, &params.lower, sizeof(params.lower));
    ...
    void run(const DepthPacket &packet, Frame *ir_frame, Frame *depth_frame, const DepthPacketProcessor::Config &config)
    {
        ...
        {
            filterPixelStage1<<<grid_size, block_size>>>(buf_a, buf_b, buf_n, buf_a_filtered,
                                                         buf_b_filtered, buf_edge_test);
        }
...
void CudaDepthPacketProcessorKernel::run(const DepthPacket &packet, Frame *ir_frame, Frame *depth_frame,
                                         const DepthPacketProcessor::Config &config)
{
